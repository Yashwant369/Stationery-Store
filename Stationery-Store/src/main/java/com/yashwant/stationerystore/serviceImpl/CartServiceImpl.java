	...
	public CartResponse addItem(String userId, AddItemRequest request) {
		...
		for(CartItem item : items)
		{
			if(item.getProduct().getProductId().equals(productId))
			{
				...
				item.setTotalPrice(quant*product.getProductDiscountPrice());
				isPresent = true;
		...
		List<CartItem>products = updatedCart.getCartItems();
		for(CartItem i : products)
		{
			CartItemResponse response = new CartItemResponse();
			response.setCartItemId(i.getCartItemId());
			response.setProductId(i.getProduct().getProductId());
			response.setProductName(i.getProduct().getProductTitle());
			response.setQuantity(i.getQuantity());
			response.setTotalPrice(i.getTotalPrice());
			it.add(response);
			
		}
		cartResponse.setCartItems(it);
		return cartResponse;
	}
